  ...
  <div class="page">
    <div class="page-header">
      ...
      <div class="page-header-actions">
        <a class="btn btn-primary btn-outline  btn-sm" href="add_journal_indexing.php?journal_id=<?php echo $journal_id; ?>">
        <i class="icon wb-plus" aria-hidden="true"></i>
        <span class="hidden-sm-down"><b>Add Journal Indexing</b></span>
      </a>
      </div>
    </div>
    ...
    <div class="page-content">
      ...
      <div class="panel">
        <header class="panel-heading mb-3">
          <h3 class="panel-title text-center">Journal Indexing List</h3>
        </header>
        <div class="panel-body">
          <table class="table table-hover table-bordered dataTable  w-full" id="exampleTableSearch">
            ...
            <tbody>
                <?php    
                $sql15="SELECT * FROM journal_indexing where journal_id='$journal_id'";
	        $result15=mysqli_query($user->db,$sql15);
	        while($indexinglist_data=mysqli_fetch_array($result15)){
               $indexing_id=$indexinglist_data["indexing_id"];
                $sql="SELECT * FROM indexing where indexing_id='$indexing_id'";
	        $result=mysqli_query($user->db,$sql);
	        $indexing_data=mysqli_fetch_array($result);
                echo '<tr>
                
                <td> <img class="brand-img" src="../edelweiss/upload/indexes/'.$indexing_data["index_logo"].'" alt="..." style="width:120px;height:60px;"></td>
                <td>'.$indexing_data["logo_description"].'</td>
                <td>'.$indexing_data["link"].'</td>
            
                 <td class="text-center">
                     <a href="deletebyid.php?delete_journalindexid='.$indexinglist_data["journal_indexing_id"].'&journal_id='.$journal_id.'" title="Delete"><i class="fa fa-trash-o"></i></a>
                 </td>
              </tr>';
            }
