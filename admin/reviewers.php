  ...
  <div class="page">
    <div class="page-header">
      <ol class="breadcrumb">
        ...
        <li class="active breadcrumb-item">Reviewers</li>
      </ol>
        ...
        <div class="col-12">
          ...
          <div class="panel">
            <div class="panel-heading">
              <h3 class="panel-title text-center">Reviewers Content</h3>
            </div>
            <div class="panel-body">
                <form action="" method="post">
                    <?php  $sql="SELECT * FROM pages WHERE page_id=3";
	                       $result = mysqli_query($user->db,$sql);
	                       $reviewer_data = mysqli_fetch_array($result);
	                       $reviewer_content= $reviewer_data['page_content'];
                    ?>
                     <textarea id="summernote" data-plugin="summernote" name="reviewer_content"> <?php echo $reviewer_content; ?></textarea>
                     <button type="submit" class="btn btn-outline btn-primary" name="reviewer_save">Save</button>
                </form>
            </div>
          </div>
...
<?php 
    if(isset($_REQUEST['reviewer_save'])){
        extract($_REQUEST);
        $result = $user->update_reviewer($reviewer_content);
        if($result){
         echo "<script>window.open('reviewers.php','_self')</script>";
        }
        else{
