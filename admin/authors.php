  ...
  <div class="page">
    <div class="page-header">
      <ol class="breadcrumb">
        ...
        <li class="active breadcrumb-item">Authors</li>
      </ol>
        ...
        <div class="col-12">
          ...
          <div class="panel">
            <div class="panel-heading">
              <h3 class="panel-title text-center">Authors Content</h3>
            </div>
            <div class="panel-body">
                <form action="" method="post">
                    <?php  $sql="SELECT * FROM pages WHERE page_id=2";
	                       $result = mysqli_query($user->db,$sql);
	                       $author_data = mysqli_fetch_array($result);
	                       $author_content= $author_data['page_content'];
                    ?>
                     <textarea id="summernote" data-plugin="summernote" name="author_content"> <?php echo $author_content; ?></textarea>
                     <button type="submit" class="btn btn-outline btn-primary" name="author_save">Save</button>
                </form>
            </div>
          </div>
...
<?php 
    if(isset($_REQUEST['author_save'])){
        extract($_REQUEST);
        $result = $user->update_author($author_content);
        if($result){
