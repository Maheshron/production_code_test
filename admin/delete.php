...
<?php include("sidebar.php"); ?>
  ...
  <div class="page">
    <?php
      //Admin delete
     if (isset($_REQUEST['admin_id']) && isset($_REQUEST['type'])) {
		 extract($_REQUEST);
	    $delete_admin = $user->del_admin($admin_id,$type);
	    if ($delete_admin) {
	        // Login Success
      ...
      if(isset($_REQUEST['articletype_id'])){
      extract($_REQUEST);
       $delete = $user->delete_articletype_id($articletype_id);
       if($delete){
        echo '<script> setTimeout(function () { swal("Success ! ","Successfully Deleted","success");}, 2000); </script>';
       }
       else{
      ...
      if(isset($_REQUEST['journal_id'])){
        extract($_REQUEST);
         $delete = $user->delete_journal_id($journal_id);
         if($delete){
             $sqlj_admin = "delete from admin where journal_id = '$journal_id'";
          $resultj_admin = mysqli_query($user->db,$sqlj_admin);
          if($resultj_admin){
           echo '<script> setTimeout(function () { swal("Success ! ","Successfully Deleted","success");}, 2000); </script>';
        ...
        if(isset($_REQUEST['delete_indexid'])){
          extract($_REQUEST);
          $delete = $user->delete_index_id($delete_indexid);
          if($delete){
           echo '<script> setTimeout(function () { swal("Success ! ","Successfully Deleted","success");}, 2000); </script>';
      ...
      if(isset($_REQUEST['subscribe_id'])){
          extract($_REQUEST);
            $sqlsubscribe = "delete from subscribe where id = '$subscribe_id'";
          $resultsubscribe = mysqli_query($user->db,$sqlsubscribe);
          if($resultsubscribe){
           echo '<script> setTimeout(function () { swal("Success ! ","Successfully Deleted","success");}, 2000); </script>';
      ...
      if(isset($_REQUEST['seopages_id'])){
          extract($_REQUEST);
          $delete = $user->delete_seopages_id($seopages_id);
          if($delete){
           echo '<script> setTimeout(function () { swal("Success ! ","Successfully Deleted","success");}, 2000); </script>';
        ...
        if(isset($_REQUEST['journalseo_id'])){
          extract($_REQUEST);
          $delete = $user->delete_innersidebar_seopages_id($journalseo_id);
          if($delete){
           echo '<script> setTimeout(function () { swal("Success ! ","Successfully Deleted","success");}, 2000); </script>';
